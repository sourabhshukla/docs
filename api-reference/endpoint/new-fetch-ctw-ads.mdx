---
title: "New Fetch CTW Ads"
openapi: "POST /meta-ads/new-fetch-ctw-ads"
---
...
2. Fetch assistantData, adAccoutData from DB and throw error if any of them is not found.

   - Also find `MetaTwoTier` document using `assisatantId` and `fbUserId` in `assistantData` and check if the ad was created through aisensy platform or not.
   - If it is not created on our Aisensy platform then simply thrown as error stating `Unable to sync meta ad account`.
...
4. Now for each ad an updated ad object is created with the help of information received using meta api.
   ...
   - An additional commission is also added in `dailyBudget` and `totalBudget` attributes.

5. The ad account document is now updated and also the ad data is also written to DB using bulkWrite and a sucess msg is then returned to the user.
